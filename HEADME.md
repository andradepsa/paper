...
This application is openly available for adaptation by the research community. To configure the software for publishing under your name, please follow these steps:

1.  **Configure API Keys:** The application requires a Google Gemini API Key and a Zenodo Personal Access Token. Both can be securely stored within the application's settings (⚙️ icon).
2.  **Update Author Information:** To ensure correct attribution, you must modify the author's name, affiliation, and ORCID number directly in the application's source code.
